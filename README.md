...
[Pełny podgląd Markdown, ze wszystkimi tabelami i linkami](regulamin/Regulamin-powolywania-reprezentacji-Polski-weteranow-w-szermierce_2026.podglad.md) jest generowany razem z DOCX; nie służy do edycji. W źródle linki i objaśnienia są zwykłym Markdown w blokach `<!-- note:identyfikator -->` … `<!-- /note -->`, w odpowiednich miejscach dokumentu. Wszystkie komórki tabel punktacji załącznika są zapisane w kanonicznym `.md` po `{{annex:points}}`; generator odczytuje je dosłownie. Nagłówek `####` nadaje tytuł kolejnej tabeli. Dane okładki, nagłówka, stopki i opisy załącznika są jawnymi polami metadanych tego samego pliku. Pola `cover_version` i `cover_date` zachowują wartości z wzorcowego DOCX; nie należy mylić ich z polami historii wersji `version` i `project_date`.
